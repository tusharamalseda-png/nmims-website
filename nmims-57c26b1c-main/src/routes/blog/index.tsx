...
import { ArrowRight, Calendar, ShieldCheck, IdCard, Wallet, BookOpen } from "lucide-react";
...
export const Route = createFileRoute("/blog/")({
  loader: async () => {
    ...
    return all.filter(
      (p) =>
        p.status === "published" && (!p.scheduledFor || new Date(p.scheduledFor).getTime() <= now),
    );
  },
  head: ({ loaderData }) => ({
    meta: [
      { title: "NMIMS Online Blog | Admissions, Fees & Eligibility Guides for Gujarat Students" },
      {
        name: "description",
        content:
          "Clear, fact-checked guides on NMIMS CDOE online degree admissions - UGC validity, ABC ID & DEB ID setup, and fees & EMI options - written for students and working professionals across Gujarat.",
      },
      { property: "og:title", content: "NMIMS Online Blog - Admission & Fees Guides" },
      {
        property: "og:description",
        content:
          "Fact-checked guides on NMIMS CDOE online degree admissions, UGC validity, ABC ID/DEB ID setup, and fees & EMI options.",
      },
    ...
    scripts: [
      ...
      },
      {
        ...
        children: JSON.stringify({
          ...
          description:
            "Admission, eligibility and fee guides for NMIMS CDOE's online degree programmes.",
  ...
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#1f1b2e_0%,#2a2440_55%,#3a2f55_100%)] py-14 sm:py-20">
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <h2 className="mt-3 font-serif text-2xl font-bold leading-tight text-white sm:text-[30px] lg:text-[34px]">
          Blogs
        </h2>
        <p className="mx-auto mt-3 max-w-2xl text-base text-white/80 sm:whitespace-nowrap">
          Three guides covering the questions we hear most often from students and parents.
        </p>
      </div>
    </section>
  );
