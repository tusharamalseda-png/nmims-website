      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Export/import site content, snapshot backups, and privacy data requests.
          </p>
        </div>
      </div>
...
function ImportExportTab() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const blob = new Blob([JSON.stringify({ exportedAt, data }, null, 2)], {
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Import failed — check the file is a valid export.",
      );
    } finally {
      setImporting(false);
    ...
    <div className="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-card">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Downloads every page, post, FAQ, testimonial, team member, logo, menu link, redirect and
          site setting as one JSON file.
        </p>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Restores from a previously exported file. Safe to re-run — pages/posts are matched by slug
          and updated only if new, everything else is only added if it doesn't already exist. Never
          deletes anything.
        </p>
        <Button
          size="sm"
...
function BackupsTab() {
  ...
  async function handleCreate() {
    setCreating(true);
    try {
      const row = await createBackupFn({
        data: { label: label || `Manual backup — ${formatDateTime(new Date())}` },
      });
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-2xl border border-border bg-card p-5 shadow-card">
        <div className="flex-1">
          ...
          <Input
            ...
            placeholder="e.g. Before homepage rewrite"
          />
        </div>
          ...
          {restoringId === b.id && (
            <div className="mt-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
              <p className="flex items-center gap-1.5 text-xs font-bold text-destructive">
                <ShieldAlert className="h-3.5 w-3.5" /> This replaces ALL current content with this
                backup. It cannot be undone.
              </p>
...
function PrivacyTab() {
  ...
  async function handleExportRequest() {
    ...
    try {
      const blob = new Blob(
        [JSON.stringify({ leads, testimonials: testimonialMatches }, null, 2)],
        { type: "application/json" },
      );
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
  ...
  async function handleErase() {
    if (
      !confirm(
        `Permanently erase the ${leads.length + testimonialMatches.length} matching record(s)? This cannot be undone.`,
      )
    )
      return;
    setBusy(true);
    try {
      const row = await fulfillDataRequestFn({
        data: {
          ...
          identifier,
          notes: null,
          eraseLeadIds: leads.map((l) => l.id),
          eraseTestimonialIds: testimonialMatches.map((t) => t.id),
        },
      });
      setLog((prev) => [row as DataRequest, ...prev]);
      setLeads([]);
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
        ...
        <p className="mb-3 text-xs text-muted-foreground">
          Search by name, email or phone across leads and testimonial submissions.
        </p>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={searching || identifier.trim().length < 2}
          >
              ...
              <>
                ...
                {testimonialMatches.map((t) => (
                  <div key={t.id} className="rounded-lg border border-border p-2.5 text-xs">
                    <p className="font-semibold text-foreground">
                      {t.name}{" "}
                      <span className="font-normal text-muted-foreground">— testimonial</span>
                    </p>
                    <p className="truncate text-muted-foreground">{t.quote}</p>
                  </div>
                ))}
                <div className="flex gap-2 pt-1">
                  <Button size="sm" variant="outline" onClick={handleExportRequest} disabled={busy}>
                    Export this data
                  </Button>
                  <Button size="sm" variant="destructive" onClick={handleErase} disabled={busy}>
                    Erase this data
                  </Button>
                </div>
              </>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
        ...
        {log.length === 0 && (
          <p className="text-xs text-muted-foreground">No requests fulfilled yet.</p>
        )}
        {log.map((r) => (
          <div
            key={r.id}
          ...
          >
            <span
              className={`rounded-full px-2 py-0.5 font-bold ${r.requestType === "erase" ? "bg-destructive/10 text-destructive" : "bg-emerald-100 text-emerald-700"}`}
            >
              {r.requestType}
            </span>
